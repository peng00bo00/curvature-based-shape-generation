...
from .util import WCS2Tangent, Tangent2WCS
...
def createValueFunc(bvh: BVH, rho: np.ndarray) -> Callable:
...
class FunctionEncoder:
    ...
    def query(self, V: np.ndarray, F: np.ndarray, valuesRef: np.ndarray) -> np.ndarray:
        ...
        assert valuesRef.shape[2] == len(self.ref.F), "Invalid value function!"
        ...
        for i in tqdm(range(len(F))):
            ray = Ray(np.zeros(3), barycenter[i])
